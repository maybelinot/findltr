...
##################################################
get_path_conf($conf_file, \$main_dir, \$main_genome_dir, \$tool_emboss, \$tool_trf, \$tool_hmmer);
...
sub get_path_conf{
	...
	if ($temp[0] eq "data_dir"){
	    if ($temp[1] !~ /\/$/){
		$temp[1] = $temp[1]."/";
	    }
	    ${$_[1]} = $temp[1];
	}elsif ($temp[0] eq "genome_dir"){
	    if ($temp[1] !~ /\/$/){
		$temp[1] = $temp[1]."/";
	    }
	    ${$_[2]} = $temp[1];
	}elsif ($temp[0] eq "sw_emboss"){
	    ${$_[3]} = $temp[1];
	}elsif ($temp[0] eq "sw_trf"){
            ${$_[4]} = $temp[1];
        }elsif ($temp[0] eq "sw_hmmer"){
            ${$_[5]} = $temp[1];
        }
    }
    close(IN);
...
sub find_sim_by_matcher{
    ...
    if ($ltr==1){
	$temp_matcher = $tool_emboss."matcher -datafile=".$tool_matrix." -asequence=".$_[0]." -bsequence=".$_[1]." -outfile=stdout -awidth3=4000 2>/dev/null";
    }else{
	$temp_matcher = $tool_emboss."matcher -asequence=".$_[0]." -bsequence=".$_[1]." -outfile=stdout -awidth3=4000 2>/dev/null";
    }
...
sub usage {
    die "Usage: find_ltr.pl --path=<main_data_dir_path> --genome=<chr_name>";
}
...
sub find_sim{#$file1, $file2
    ...
    my @result=();
    my $temp_matcher = $tool_emboss."matcher -datafile=".$tool_matrix." -asequence=".$_[0]." -bsequence=".$_[1]." -outfile=stdout -awidth3=4000 2>/dev/null";
    my $str_result = `$temp_matcher`;
    my @line = split(/\n/, $str_result);
...
sub find_domain{ #$file3, $file4
    ...
    system($tool_emboss."transeq -sequence ".$_[0]." -outseq ".$_[1]." -frame=6  2>/dev/null");
	...
	my $temp_tool=$tool_hmmer."hmmsearch -E 0.000001 ".$tool_pfam.$rt[$j]." ".$_[1];
	my $str = `$temp_tool`;
	...
	for (my $j=0; $j<=$#pf; $j++){
	    ...
	    my $temp_tool=$tool_hmmer."hmmsearch -E 0.000001 ".$tool_pfam.$pf[$j]." ".$_[1];
	    my $str = `$temp_tool`;
